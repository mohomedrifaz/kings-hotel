...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (
        isset($_POST['s_name'])
        && isset($_POST['s_address']) && isset($_POST['s_contact'])
        ...
        $name = validate($_POST['s_name']);
        $address = validate($_POST['s_address']);
        $contact = validate($_POST['s_contact']);
        ...
        if (empty($name)) {
            header("Location: add-staff.php?error= Name is required&$user_data");
            exit();
        } else if (empty($address)) {
            header("Location: add-staff.php?error= Address is required&$user_data");
            exit();
        } else if (empty($contact)) {
            header("Location: add-staff.php?error= Contact is required&$user_data");
            exit();
        } else if (empty($email)) {
            header("Location: add-staff.php?error= Email is required&$user_data");
            exit();
        } else if (empty($password)) {
            header("Location: add-staff.php?error=Password is required&$user_data");
            exit();
        } else {
            //Hashing the password
            $password = md5($password);

            $sql2 = "insert into staff(s_name,s_address,s_contact,email,password) values('$name','$address','$contact','$email','$password')";
            $result2 = mysqli_query($con, $sql2);
            if ($result2) {
                header("Location: add-staff.php?success=Your account was created succesfully");
                exit();
            } else {
                header("Location: add-staff.php?error=Unknown error occured&$user_data");
                exit();
            }
        }
    } else {
                    ...
                    <div class="col-md-12">
                        <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" placeholder="Enter Name" name="s_name">
                            </div>
                            <div class="form-group">
                                <label for="address">Address</label>
                                <input type="text" class="form-control" placeholder="Enter Address" name="s_address">
                            </div>

                            <div class="form-group">
                                <label for="contact">Contact No</label>
                                <input type="text" class="form-control" placeholder="Enter Contact" name="s_contact">
                            </div>
                                            ...
                                            <h4 class="panel-title">
                                                <a data-toggle="collapse" data-parent="#accordion" href="#collapseTwo">
                                                    <button class="btn btn-default" type="button">
                                                        View Staff <span class="badge"></span>
                                                    </button>
                                                </a>
                                            </h4>
                                        </div>
                                                        ...
                                                        <div class="table-responsive">
                                                            <table class="table">
                                                                ...
                                                                <tbody>
                                                                    ...
                                                                    <?php
                                                                    $tsql = "select * from staff";
                                                                    $tre = mysqli_query($con, $tsql);
                                                                    while ($trow = mysqli_fetch_array($tre)) { {
                                                                            echo "<th>" . $trow['s_id'] . "</th>";
                                                                            echo "<th>" . $trow['s_name'] . "</th>";
                                                                            echo "<th>" . $trow['s_contact'] . "</th>";
                                                                            echo "<th>" . $trow['s_address'] . "</th>";
                                                                            echo "<th>" . $trow['email'] . "</th>";
                                                                            echo "<th onclick=\"window.location='edit_staff.php?id=" . $trow['s_id'] . "';\">" ?><button
                                                                                type="button"
                                                                                class="print-none btn btn-success">Edit</button>
                                                                            <?php
                                                                            echo "<th onclick=\"deleteStaff(" . $trow['s_id'] . ");\">" ?>
                                                                            <button type="button"
                                                            ...
                                                            {
                                                                global $con;
                                                                mysqli_query($con, "DELETE FROM staff WHERE s_id=$id");
