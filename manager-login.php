...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['email']) && isset($_POST['password'])) {
        ...
        if (empty($email)) {
            header("Location: manager-login.php?error=Email is required");
            exit();
        } else if (empty($password)) {
            header("Location: manager-login.php?error=Password is required");
            exit();
        } else {
            ...
            if (mysqli_num_rows($result) === 1) {
                ...
                if ($row['email'] === $email && $row['password'] === $password) {
                    ...
                    $_SESSION['contact'] = $row['m_contact'];
                    header("Location: manager-dashboard");
                    exit();
                } else {
                    header("Location: manager-login.php?error=Incorect User name or password");
                    exit();
                }
            } else {
                header("Location: manager-login.php?error=Incorect User name or password");
